	...
	<main class="checkout-container">
		...
		<form action="checkout" method="post" class="checkout-form">
				...
				<div class="section-content">
					...
					<div class="login-prompt">
						...
						<div class="login-buttons">
							<a href="login.jsp" class="btn secondary-btn">Log In</a> <a
								href="registration.jsp" class="btn primary-btn">Sign Up</a>
						</div>
					</div>
					<% } %>
				</div>
			...
			<div class="checkout-section delivery-section">
				...
				<div class="section-content">
					<div class="form-group">
						<label for="deliveryAddress">Where should we deliver your
							order?</label>
						<textarea id="deliveryAddress" name="deliveryAddress" rows="4"
							placeholder="Enter your complete address with street, landmark, and city..."
							required></textarea>
					</div>
				</div>
			</div>
			...
			<div class="checkout-section payment-section">
				...
				<div class="section-content">
					<div class="form-group">
						<label for="paymentMethod">Choose Payment Option</label> <select
							id="paymentMethod" name="paymentMethod" class="disabled-select">
							<option value="cash_on_delivery" selected>Cash on
								Delivery</option>
							<option value="debit_card" disabled>Debit Card</option>
							<option value="credit_card" disabled>Credit Card</option>
							<option value="upi" disabled>UPI Payment</option>
						</select>
						<p class="disabled-message">Other payment methods are coming
							soon!</p>
					</div>
				</div>
			</div>
			...
			<div class="order-summary">
				...
				<div class="summary-item">
					<span>Cart Total</span> <span>₹<%= String.format("%.2f", totalCartAmount) %></span>
				</div>
				<div class="summary-item">
					<span>Delivery Fee</span> <span>₹40.00</span>
				</div>
				<div class="summary-item total">
					<span>Total Amount</span> <span>₹<%= String.format("%.2f", totalCartAmount + 40.00) %></span>
				</div>

				<!-- Place Order Button in Summary -->
				<% if (isLoggedIn2 && cart2 != null && !cart2.getItems().isEmpty()) { %>
				<button type="submit" class="btn primary-btn large-btn">Place
					Order</button>
				<% } else { %>
				<p class="disabled-order-message">Please log in and add items to
					cart</p>
				<button type="button" class="btn primary-btn large-btn disabled-btn"
